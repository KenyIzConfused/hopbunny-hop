...
WIDTH = 1200
HEIGHT = 440
PAD_L, PAD_R, PAD_T, PAD_B = 88, 48, 78, 78
Y_STEP = 100
MAX_X_MONTH_LABELS = 8
...
RELEASE_SUBJECT_RE = re.compile(
    r"(?i)^(release\b|release:|feat:\s*ship\s+|feat\(release\)|feat:\s*v\d|feat:\s*initial\b)"
)
...
def month_starts_in_range(start_d: date, end_d: date) -> list[date]:
    """First day of every calendar month that intersects [start_d, end_d]."""
    months: list[date] = []
...
def select_even_months(months: list[date], max_labels: int = MAX_X_MONTH_LABELS) -> list[date]:
    """Keep first/last; pick remaining at even index steps (no random skips)."""
    if len(months) <= max_labels:
        return list(months)
    if max_labels <= 2:
        return [months[0], months[-1]]
    out: list[date] = []
    for i in range(max_labels):
        idx = round(i * (len(months) - 1) / (max_labels - 1))
        out.append(months[idx])
    deduped: list[date] = []
    for m in out:
        if not deduped or deduped[-1] != m:
            deduped.append(m)
    if deduped[-1] != months[-1]:
        deduped[-1] = months[-1]
    return deduped
...
def equal_spaced_x(
    ...
    """Equal visual centers for axis labels (independent of calendar day widths)."""
    if count <= 1:
        return chart_x + chart_w / 2
    usable = max(0.0, chart_w - 2.0 * inset)
    return chart_x + inset + (index / (count - 1)) * usable
...
def render_svg(
    ...
    # X labels: consecutive months (or evenly thinned), equal visual centers.
    # Always text-anchor=middle — start/end anchors made equal slots look uneven.
    # Star series stays time-linear; month labels do not track calendar day widths.
    months = month_starts_in_range(start_d, end_d)
    shown = select_even_months(months, MAX_X_MONTH_LABELS)
    x_labels: list[tuple[float, str]] = []
    n_shown = len(shown)
    # ~half width of "MMM YYYY" at 12px mono so edge labels stay inside the SVG.
    label_inset = 36.0
    for i, m in enumerate(shown):
        x = equal_spaced_x(i, n_shown, chart_x, chart_w, inset=label_inset)
        x_labels.append((x, month_label(m)))
    ...
    # Axis titles
    axis_title_y = (
        f'<text x="18" y="{chart_y + chart_h / 2:.2f}" text-anchor="middle" '
        f'fill="{pal["axis"]}" font-family="Consolas, ui-monospace, monospace" '
        f'font-size="11" font-weight="700" letter-spacing="2" '
        f'transform="rotate(-90 18 {chart_y + chart_h / 2:.2f})">STARS</text>'
    )
    axis_title_x = (
        f'<text x="{chart_x + chart_w / 2:.2f}" y="{HEIGHT - 14}" text-anchor="middle" '
        f'fill="{pal["axis"]}" font-family="Consolas, ui-monospace, monospace" '
        f'font-size="11" font-weight="700" letter-spacing="2">DATE</text>'
    )
    ...
    x_label_svg = "".join(
        f'<line x1="{x:.2f}" y1="{chart_y + chart_h}" x2="{x:.2f}" y2="{chart_y + chart_h + 6}" '
        f'stroke="{pal["axis"]}" stroke-width="1" opacity="0.55"/>'
        f'<text x="{x:.2f}" y="{HEIGHT - 36}" text-anchor="middle" '
        f'fill="{pal["axis"]}" font-family="Consolas, ui-monospace, monospace" '
    ...
    y_label_svg = "".join(
        ...
        f'<text x="{PAD_L - 14}" y="{y + 4:.2f}" text-anchor="end" '
        f'fill="{pal["axis"]}" font-family="Consolas, ui-monospace, monospace" '
